...
class PromotionCodesMixin:
    def create_promotion_code(
        ...
        if minimum_amount and not currency:
            raise Exception('Currency is required if minimum_amount is provided.')
        ...
        return self.request(
            '/promotion-codes', POST, payload=payload, idempotency_key=idempotency_key
        )
    ...
    def update_promotion_code(
        self, id=None, active=None, metadata=None, idempotency_key=None
    ):
        if not id:
            raise Exception('Promotion Code Id is required.')
